...
- 词表提供可视化列表，支持新增、编辑、删除、批量/全部确认、清理重复
- 可在已有词表基础上对另一本书“提取更多词表”，自动跳过已有词条（已确认词条不会被覆盖）
- 提取词表时 LLM 会为每个词条自动建议“可能译法”；也可对已加载词表点“完善信息”（不传原文）补充可能译法与备注
- 翻译时把词表注入提示词，并在结果中做保底替换，保证全书译名一致
- 词条可填写“可能译法”（半角逗号分隔）：提示词只提供人工确认译名；回传检出可能误译时自动替换为确认译名（仅已确认词条生效，单字变体自动忽略，1-2 字变体保存时会提醒）
- 可能译法若是全名的子串（如“范镇”）视为合法短称/昵称，默认不替换以保留亲昵称呼；可勾选“替换短称”强制替换为全名
- 词表操作栏提供“判断短称”按钮：把含短称的词条发给 LLM 自动判定“替换短称”开关，人工复核确认即可
