...
- [ ] 阶段一：新增「设置」tab；迁移左侧 API / 翻译参数；补全 `.gui_config.json` 持久化（`app_config`）
- [ ] 阶段二：LLM API 预设列表（增删改、上移下移、一键切换）；失败块弹窗用预设下拉
- [ ] 阶段三：词表/输入列表字体大小、行高，改动即时生效
- [ ] 阶段四：词表采样预算、翻译容错、外观主题、输出编码等高级项；与 `--config` 命令行对齐
