...
EXTRACTION_SYSTEM = """你是一名韩语小说编辑，负责从原文中提取需要在全书中保持一致的专有名词。
只提取真正会影响阅读一致性的词：人名、地名、组织、种族、特殊称谓、作品内设定术语、固定物品名。
不要提取普通动词、形容词、常见词。
只输出 JSON，不要输出解释或 Markdown。格式：
{"entries":[{"ko":"韩文原词","zh":"建议中文译名","kind":"person|place|org|term|title","note":"简短备注"}]}
如果某词不确定，zh 也要给出你认为最合理的译名，note 里说明待确认。"""
...
def extract_glossary_with_llm(llm: LLMClient, sample_text: str, limit: int) -> Glossary:
    ...
    user = (
        "请从下面这段小说样章中提取专有名词词表。最多提取 "
        f"{limit} 条，按出现频率和重要性排序。原文如下：\n\n{sample[:200000]}"
    )
    messages = [
